...
public class BinarySearchTree<E extends Comparable<E>>
         extends BinaryTree<E>implements Iterable<Integer>{
    // Data Fields
    ...
    public Iterator<Integer> iterator() {
        if(this.isLeaf())//if no child
            return  null;
        return new myIterator<Integer>(this);       
    }

    /**
     * my own implemented iterator class
     * @param <Integer> 
     */ 
    private class myIterator<Integer> implements Iterator<Integer> {
        private BinarySearchTree<E>array=new BinarySearchTree<E>();        
        private  myIterator(BinarySearchTree<E> temp) {          
            array=temp;
        }       
        @Override
        public Integer next() {
            try{
                if(array.root==null)
                    throw new NullPointerException("There is no node.You can't use next() method!");                      
            }catch(NullPointerException e) {
                System.err.println(e.getMessage());
                System.exit(0);
            }
            return (Integer)findMin(array.root);
        }    
        @Override
...
// Insert solution to programming exercise 1, section 4, chapter 6 here
     ...
     * 
     * @param parent
     * @return smallest data to next() method in myIterator class
     */
    private E findMin(Node<E> parent) {   
        while(parent.left!=null){
            parent=parent.left;
        }        
        E cleaner=parent.data;
        delete(parent.data);
        return cleaner;     
    }
    /*</listing>*/
...
// Insert solution to programming exercise 3, section 4, chapter 6 here
    /**
     * BinarySearchTree class's main method
     * @param args 
     */
    public static void main(String[] args) {
        ...
        while(iter.hasNext())//print has next oldugu surece        
            System.out.println(iter.next());
        System.out.println("-------------------------------------");
        //System.out.println("If we want next again.We will see the following message...\n");
        //System.out.println(iter.next());
